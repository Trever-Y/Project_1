...
We created a function that has an optional argument that allows the user to specify the name of the column representing the value (which is `Enrollment Value` in our case)
```{r}
#Create the function
...
### Questions 4-6 in 1 function

In order to create two new tibbles from the output from step 3 and apply steps 5-6, we need to combine a few functions.
```{r}
...
We want to combine our data from two different URLs. We will create a single short function that takes in the results of the two calls in our wrapper function. 
```{r}
...
We run our wrapper function on the two EDU data sets and inspect the results.
